...
def frameProcess():
    for dirs in os.walk('temp1'):
        for d2rs in dirs[1]:
            ...
            data = os.listdir(f'temp1/{d2rs}')  # 循环切片图片转换为1位图
            for dat in data:
                conv.conv3(f'temp1/{d2rs}/{dat}', f'temp2/{d2rs}/{dat}')  # 转换成灰度图像
            d3ta = {}  # 定义一个字典来存储数据
            data4 = os.listdir(f'temp2/{d2rs}')  # 获取一位图
            d3ta.update({'fc_len': str(len(data4))})  # 获取所有帧数添加到字典
            for dat4 in data4:  # 开始循环
                res = conv.createCode(f'temp2/{d2rs}/{dat4}')  # 1位图转换到字码
...
if __name__ == '__main__':
    try:
        ...
        print('读取配置...')
        ver = config['ver']
        print(f'欢迎使用ESP32 P2layer 视频转换工具(单色视频专用)>>当前版本{ver}')
